...
internal class TracingViewModel(
    ...
    val screenState: StateFlow<ScreenState<TracingUiState>> =
        unitRepository.observeLessons(unitId)
            .map { lessons ->
                if (lessons.isEmpty()) {
                    ScreenState.Error(message = Res.string.error_no_data)
                } else {
                    ScreenState.Idle(TracingUiState(lessons = lessons.toImmutableList()))
                }
            }
            .catch { throwable ->
    ...
    private companion object {
        const val TAG = "TracingViewModel"
        const val SUBSCRIPTION_TIMEOUT_MS = 5_000L
    }
}

@Immutable
internal data class TracingUiState(
